    ...
    const fetchLeaves = async () => {
      ...
      try {
        ...
        if (err.response?.status === 401) {
          setError("Session expired. Please login again."); navigate('/login');
        } else {
                  ...
                  {currentLeaves.map((leave) => {
                    ...
                    return (
                      <tr key={leave.id} style={styles.tr}>
                        ...
                        <td style={{...styles.td, maxWidth: '200px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}} title={leave.reason}>
                          {leave.reason}
                        </td>
                        <td style={styles.td}>
                          <span style={{
  ...
  View
            ...
            {totalPages > 1 && (
              <div style={styles.pagination}>
                <button style={currentPage === 1 ? styles.pageBtnDisabled : styles.pageBtn} onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1}>
                  Previous
                </button>
                <span style={styles.pageText}>Page {currentPage} of {totalPages}</span>
                <button style={currentPage === totalPages ? styles.pageBtnDisabled : styles.pageBtn} onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages}>
                  Next
                </button>
              </div>
            )}
...
const styles = {
  ...
  container: { padding: '40px 20px', maxWidth: '1300px', margin: '0 auto', width: '100%', boxSizing: 'border-box' },
  ...
  th: { background: '#f8fafc', padding: '18px 24px', color: '#64748b', fontWeight: '700', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.5px', borderBottom: '1px solid #e2e8f0', whiteSpace: 'nowrap' },
  tr: { transition: 'background 0.2s' },
  td: { padding: '18px 24px', color: '#475569', fontSize: '0.95rem', borderBottom: '1px solid #f1f5f9' },
  tdBold: { padding: '18px 24px', color: '#1e293b', fontSize: '0.95rem', fontWeight: '600', borderBottom: '1px solid #f1f5f9' },
  link: { color: '#3b82f6', textDecoration: 'none', fontWeight: '600' },
  pagination: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 30px', background: 'white', borderTop: '1px solid #f1f5f9' },
  pageText: { color: '#64748b', fontWeight: '500', fontSize: '0.9rem' },
  pageBtn: { padding: '8px 16px', background: 'white', border: '1px solid #cbd5e1', borderRadius: '8px', cursor: 'pointer', color: '#334155', fontWeight: '600' },
  pageBtnDisabled: { padding: '8px 16px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', color: '#94a3b8', fontWeight: '600', cursor: 'not-allowed' },
};
